...
if($con)
{
    ...
    if (isset($_POST['submit-btn']))
    {
            ...
            $result = mysqli_query($con , "select *from staff where email='$email' and login_password='$pass' and privilege = 1");

            $row = mysqli_fetch_array($result);
            if ($row)
            {
              $_SESSION['adm_id'] = $row['staff_id'];
              header('Location:./admin_panel.php'); 
            }
            else
            {
                include './phpActionFiles/login_fail.php';
